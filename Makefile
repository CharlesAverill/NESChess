...
$(BUILD)/crt0.o: $(CL) $(SRC)/crt0.S $(SRC)/nesdoug.S $(ASSET_FILES) $(wildcard $(LIB)/neslib/*.s $(LIB)/neslib/*.sinc) #$(BGM) $(SFX) $(ASSET_FILES)
	$(CL) -t nes -Oisr -c $(SRC)/crt0.S
	@mv $(SRC)/crt0.o $(BUILD)

$(BUILD)/%.S: $(SRC)/%.c $(H_FILES) $(CC)
	mkdir -p $(dir $@)
	$(CC) $(CFLAGS) $< -o $@
